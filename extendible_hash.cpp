...
ExtendHash::Directory::Directory(int size)
{
    ...
    numSigBits = computeSigBits(size);
}
...
void ExtendHash::doubleDirectory()
{
    ...
    for(int i = 0; i < temp.size(); i++){
        temp.pointers[i] = directory.pointers[i/2]; //each element in the new directory = (i/2)th element in old directory; make some buckets have 2 pointers each
    }
